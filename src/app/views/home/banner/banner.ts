...
// ❌ elimina estos imports estáticos
// import { gsap } from 'gsap';
// import { ScrollTrigger } from 'gsap/ScrollTrigger';

// ✅ tipos solamente (no generan código en el bundle, se eliminan en compilación)
...
export class Banner {
  ...
  private readonly destroyRef = inject(DestroyRef);
  private readonly isBrowser = typeof window !== 'undefined';
  private parallaxTrigger: ScrollTriggerType | undefined;
  private introTween: ReturnType<typeof GsapType.to> | undefined;
  private readonly motionState = {
    progress: 0,
    introOffset: 120,
    backgroundIntroScale: 1.12,
  ...
  private async createScrollParallax(): Promise<void> {
    if (!this.isBrowser) {
      return;
    }
    ...
    // 🔑 carga async — solo se descarga cuando este componente realmente renderiza
